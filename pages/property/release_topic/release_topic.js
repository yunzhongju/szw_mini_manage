...
import {publishCommunityVoiceAPI} from '../../../config/newsAPI/PVoice_api'
import {img_sec_check,getAccessToken} from '../../../config/newsAPI/token'
var app = getApp();
Page({
    data: {
        access_token:'',
    ...
    communityvoiceAdd: function() {
        let that = this;
        let communityId=that.data.community?that.data.community:that.data.street
        console.log('formData',that.data)
        that.publishCommunityVoice(that.data.title,that.data.content,JSON.stringify(that.data.image_path),that.data.navigation_id,communityId)
    ...
    uploadDIY: function(filePaths, successUp, failUp, i, length) {
        var that = this;
        // img_sec_check(that.data.access_token,filePaths[i],res=>{
    ...
    changeData: function(options) {
        let that = this;
        that.setData({
            "province": options[0].id,
            "city": options[1].id,
            "area": options[2].id,
            "street": options[3].id,
            "community": options[4].id,
            communityDetails:options[options.length-1]?options[options.length-1]:options[options.length-1]
        })
    ...
    onShow: function() {
        ...
        common.getChannelList(3).then(res=>{
            that.setData({
                getChildTreeListByParentID:res.list
            })
        })
        // that.getUploadImageToken();
        common.getUpToken('image').then(token=>{
            console.log('token',token)
            that.setData({
                getUploadImageToken:token
            })
        })
    },
    onHide: function() {
    ...
    //发布话题
    publishCommunityVoice:function(title,content,imagePath,channelId,community){
        let params={
            title:title,
            content:content,
            imagePath:imagePath,
        ...
        publishCommunityVoiceAPI(params).then(res=>{
            console.log('发布话题',res)
            if(res.status==1){
                this.refreshBeforePage()
            }
        })
    }
})